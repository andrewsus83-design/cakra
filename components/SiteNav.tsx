...
export function SiteNav() {
  ...
  const overHero = isHero && !scrolled;
    ...
    <>
      <header
        style={{
          ...
          background: overHero ? "transparent" : "color-mix(in oklab, var(--bg) 86%, transparent)",
          backdropFilter: overHero ? "none" : "blur(10px)",
          borderBottom: `1px solid ${overHero ? "transparent" : "var(--line)"}`,
        }}
      >
        <div className="wrap" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", height: 86 }}>
          <Link href="/" style={{ display: "flex", alignItems: "center", gap: 6, textDecoration: "none", color: txt }}>
            <CakraMark size={46} />
            <span className="hand" style={{ fontSize: "2.95rem", fontWeight: 700, lineHeight: 1 }}>cakra</span>
          </Link>
        ...
        <style>{`@media (max-width: 900px){ .cakra-navlinks{ display:none !important; } }`}</style>
      </header>
      {!isHero && <div style={{ height: 86 }} />}
    </>
  );
}
