// Shared "Kirana template" (.kir) design system — used by the sample site (/demo)
// and its area spoke pages (/demo/area/[slug]) so they stay visually identical.
export const KIR_CSS = `
        .kir{
        ...
        .k-area{ font-family:var(--k-font-display), serif; font-size:1.15rem; padding:.6rem 1.4rem; border:1px solid var(--k-line-2); border-radius:999px; color:var(--k-ink); background:var(--k-surface); transition:.15s; text-decoration:none; }
        ...
        .k-crumb{ display:flex; gap:8px; align-items:center; flex-wrap:wrap; font-size:.85rem; color:rgba(255,255,255,.75); margin-bottom:18px; }
        .k-crumb a{ color:rgba(255,255,255,.75); text-decoration:none; }
        .k-crumb a:hover{ color:#fff; }
        .k-arealist{ display:grid; grid-template-columns:repeat(auto-fill,minmax(240px,1fr)); gap:14px; }
        .k-arealink{ display:flex; align-items:center; justify-content:space-between; gap:10px; padding:16px 18px; border:1px solid var(--k-line); border-radius:var(--k-r-sm); background:var(--k-surface); text-decoration:none; color:var(--k-ink); font-weight:600; transition:.15s; }
        .k-arealink:hover{ border-color:var(--k-emerald); color:var(--k-emerald); transform:translateY(-2px); }
        .k-arealink[aria-current="page"]{ border-color:var(--k-emerald); background:color-mix(in oklab, var(--k-emerald) 8%, var(--k-surface)); color:var(--k-emerald); }
